...
function changeInputClass(obj) {
    if (obj.val() != '') {
        obj.addClass("notEmptyInput");
    }
    else {
        obj.removeClass("notEmptyInput");
    }
}

function autoChangeInputLabelPosition() {
    $.each($('.form-group input'), function () {
        changeInputClass($(this));
    });
}

//Whenever input value is being changed, repective class will be given
$('.form-group input').change(function () {
    changeInputClass($(this));
});

//Whenever documents loads, respective input class is given
setTimeout(function () {
    autoChangeInputLabelPosition();
}, 400);

// Removing alert messages automatically
setTimeout(function () {
...
function sidebarCollapse() {
    ...
    var index3 = arguments[2];
    $('nav>.acc-menu:first-child>li:nth-child(' + index1 + ')').addClass('open');
    $('nav>ul>.open>ul').css({'display': 'block'});
    if ($('.acc-menu:first-child>li.open>ul>li:nth-child(' + index2 + ')').hasClass('hasChild')) {
        $('.acc-menu:first-child>li.open>ul>li:nth-child(' + index2 + ')').addClass('open');
        $('.acc-menu:first-child>li.open>ul>li:nth-child(' + index2 + ')').find('>ul').css({'display': 'block'});
        $('.acc-menu:first-child>li.open>ul>li:nth-child(' + index2 + ')').find('>ul>li:nth-child(' + index3 + ')').addClass('active');
    }
    else {
        $('.acc-menu:first-child>li.open>ul>li:nth-child(' + index2 + ')').addClass('active');
    }
}